...
class SVM:
    ...
    def tune_and_eval(self, results_file,
                      params=[{'C': [1000, 500, 200, 100, 50, 20, 10, 5, 2, 1, 0.2, 0.5, 0.01, 0.02, 0.05, 0.001],
                               ...
                               'loss': ['l2'], 'class_weight': ['balanced', None]}], njobs=50):
...
class LogRegression:
    ...
    def tune_and_eval(self, results_file,
                      params=[{'C': [1000, 500, 200, 100, 50, 20, 10, 5, 2, 1, 0.2, 0.5, 0.01, 0.02, 0.05, 0.001],
                               'penalty': ['l1'], "tol": [1e-06, 1e-04], 'dual': [False, True], "fit_intercept": [True],
                               'class_weight': ['balanced', None], 'solver': ['liblinear']}], njobs=50):
        CV = KFoldCrossVal(self.X, self.Y, folds=10)
...
class RFClassifier:
    ...
    def tune_and_eval(self, results_file, params=None, feature_names=None, njobs=50):
        '''
        Tune, evaluate, extract features (if a list of features are provided)
        :param results_file:
        :param params:
...
class KNN:
    ...
    def tune_and_eval(self, results_file, params=None, njobs=50):
        if params is None:
