...
class PredefinedFoldCrossVal(CrossValidator):
    ...
    def tune_and_evaluate(self, estimator, parameters, cv_inner=5, score='f1_macro', n_jobs=-1, file_name='results',
        ...
        [nested_scores, cv_dicts, label_set, conf, label_set, best_score_, best_estimator_,
             cv_results_, best_params_, (cv_predictions_pred, cv_predictions_trues, isolates), (Y_test_pred, Y_test)] = FileUtility.load_obj(filename)
...
class PredefinedFoldCrossVal(CrossValidator):
    ...
    def tune_and_evaluate(self, estimator, parameters, cv_inner=5, score='f1_macro', n_jobs=-1, file_name='results', NUM_TRIALS=3):
        '''
        :param estimator:
        :param parameters:p
        :param score:
        ...
        for i in tqdm.tqdm(range(NUM_TRIALS)):
            ...
            # parameter search and scoring
            self.greed_search = GridSearchCV(estimator=estimator, param_grid=parameters, cv=inner_cv, scoring=self.scoring, 
                                         refit=score, error_score=0, n_jobs=n_jobs,verbose=0)
        ...
        # save in file
        FileUtility.save_obj(file_name,
                             [self.nested_scores,cv_dicts, label_set, conf, label_set, self.greed_search.best_score_, self.greed_search.best_estimator_,
