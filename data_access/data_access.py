...
class DataAccess(object):
    ...
    def make_labels(self):
        ...
        self.phenotypes = [x.split('_')[0] for x in rows[0].rstrip().split('\t')[1::]]
        # init
        for drug in self.phenotypes:
            self.phenotype2labeled_strains_mapping[drug] = []
        mapping = {'0': 0, '0.0': 0, '1': 1, '1.0': 1, '': 0.5}
        # only consider non-empty values
        for isolate, resist_vec in self.strain2labelvector.items():
            for idx, val in enumerate(resist_vec):
                if val in mapping:
                    self.phenotype2labeled_strains_mapping[self.phenotypes[idx]].append((isolate, mapping[val]))
        # generate dict of labels for each class
        for drug in self.phenotypes:
            self.phenotype2labeled_strains_mapping[drug] = dict(self.phenotype2labeled_strains_mapping[drug])
    ...
    def get_new_labeling(self, mapping={'0': 0, '0.0': 0, '1': 1, '1.0': 1}):
        ...
        for isolate, resist_vec in self.strain2labelvector.items():
            for idx, val in enumerate(resist_vec):
                if val in mapping:
                    new_drug2labeled_isolates_mapping[self.phenotypes[idx]].append((isolate, mapping[val]))
